...
class AdminController extends Controller
{
    ...
    public function publishJob(Job $job) {
        ...
        $job->save();
        return redirect('data-lowongan/detail/' . $job->id)->with('swal', [
            'icon' => 'success',
            'title' => 'Lowongan Dipublikasikan',
            'text' => 'Status lowongan diubah menjadi published.'
        ]);
    }

    public function unpublishJob(Job $job) {
        $job->status = 'draft';
        $job->save();
        return redirect('data-lowongan/detail/' . $job->id)->with('swal', [
            'icon' => 'success',
            'title' => 'Lowongan Dikembalikan ke Draft',
            'text' => 'Status lowongan diubah menjadi draft.'
        ]);
    }

    public function deleteJob(Job $job) {
        $job->applyJobs()->delete();
        $job->delete();
        return redirect('data-lowongan')->with('swal', [
            'icon' => 'success',
            'title' => 'Lowongan Dihapus',
            'text' => 'Lowongan dan data pelamar terkait telah dihapus.'
        ]);
    }

    public function jobApplicants(Job $job) {
        return view('Admin.Job.applicants', [
            'job' => $job->load('company', 'applyJobs.user')
        ]);
    }
}
