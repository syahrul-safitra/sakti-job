                ...
                <form action="{{ url('company-lowongan/' . $job->id) }}" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="tipe" class="form-select @error('tipe') is-invalid @enderror">
                            ...
                            <option value="umkm" @selected(old('tipe', $job->tipe) === 'umkm')>UMKM</option>
                            <option value="perusahaan" @selected(old('tipe', $job->tipe) === 'perusahaan')>Perusahaan</option>
                            <option value="swasta" @selected(old('tipe', $job->tipe) === 'swasta')>Swasta</option>
                            <option value="butik" @selected(old('tipe', $job->tipe) === 'butik')>Butik</option>
                        </select>
                        @error('tipe')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
