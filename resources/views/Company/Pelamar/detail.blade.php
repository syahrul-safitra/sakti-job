...
@push('styles')
<style>
.dp-wrap .card{ border-radius:16px; }
.dp-wrap .muted{ color: rgba(255,255,255,.65); }
.dp-wrap .avatar{
...
.dp-wrap .doc-item a{ text-decoration:none; }
...
.dp-toast{
    ...
    background: rgba(20,20,20,.92); color: rgba(255,255,255,.9);
    box-shadow: 0 .75rem 1.5rem rgba(0,0,0,.35);
    display:none;
}
.dp-toast.show{ display:block; }
.dp-toast .title{ font-weight:800; font-size:.95rem; }
.dp-toast .desc{ font-size:.85rem; opacity:.85; }
</style>
@endpush
                            ...
                            <div class="d-flex align-items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <div class="mt-3 d-flex flex-wrap gap-2">
                                        @if($wa)
                                            <a class="btn btn-sm btn-success btn-soft" target="_blank" href="{{ $wa }}">
                                                <i class="bi bi-whatsapp me-1"></i> WhatsApp
                                            </a>
                                        @endif
                                        @if($phone)
                                            <a class="btn btn-sm btn-outline-secondary btn-soft" href="tel:{{ $phone }}">
                                                <i class="bi bi-telephone-outbound me-1"></i> Telepon
                                            </a>
                                        @endif
                                        @if($email && $email !== '-')
                                            <a class="btn btn-sm btn-outline-primary btn-soft" href="mailto:{{ $email }}">
                        ...
                        @if(!empty($skills))
                            <div class="mt-4">
                                ...
                                <div class="d-flex flex-wrap gap-2">
                                    @foreach($skills as $s)
                                        <span class="chip"><i class="bi bi-lightning-charge"></i>{{ $s }}</span>
                                    @endforeach
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="row g-3">
                            <div class="col-12 col-md-6">
                                <div class="doc-item">
                                    <div class="d-flex align-items-start justify-content-between gap-2">
                                        <div>
                                            <div class="fw-semibold"><i class="bi bi-file-earmark-text me-1"></i> CV / Resume</div>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="doc-item">
                                    <div class="d-flex align-items-start justify-content-between gap-2">
                                        <div>
                                            <div class="fw-semibold"><i class="bi bi-link-45deg me-1"></i> Portofolio</div>
                                            <div class="text-muted small">Link karya/website</div>
                                        </div>

                                        @if($portfolio)
                                            <a class="btn btn-sm btn-outline-secondary btn-soft" target="_blank" href="{{ $portfolio }}">
                                                Buka
                                            </a>
                                        @else
                                            <span class="badge bg-light text-dark">Tidak ada</span>
                            ...
                            <div class="col-12 col-md-6">
                                <div class="doc-item">
                                    <div class="d-flex align-items-start justify-content-between gap-2">
                                        ...
                                        @if($linkedin)
                                            <a class="btn btn-sm btn-outline-secondary btn-soft" target="_blank" href="{{ $linkedin }}">
                                                Buka
                                            </a>
                                        @else
                                            <span class="badge bg-light text-dark">Tidak ada</span>
            ...
            <div class="col-12 col-lg-4">
                ...
                <div class="card mb-3">
                    <div class="card-body">
                        ...
                        <form id="statusForm" action="{{ url('company-applyjob/update-status/' . $apply->id) }}" method="POST">
                            @csrf

                            <input type="hidden" name="keterangan" id="keteranganHidden" value="{{ $apply->keterangan }}">

                            <label class="form-label">Status</label>
                            <select name="status" id="statusSelect" class="form-select mb-3">
                                <option value="pending"  {{ ($apply->status ?? 'pending') === 'pending' ? 'selected' : '' }}>Pending</option>
                                <option value="accepted" {{ ($apply->status ?? 'pending') === 'accepted' ? 'selected' : '' }}>Diterima</option>
                                <option value="rejected" {{ ($apply->status ?? 'pending') === 'rejected' ? 'selected' : '' }}>Ditolak</option>
                            </select>

                            <button type="submit" id="saveStatusBtn" class="btn btn-success w-100">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="mt-2 d-flex gap-2">
                            <a href="{{ url('lowongan/detail/' . $job->id) }}" class="btn btn-sm btn-outline-primary btn-soft">
                                Lihat Lowongan
                            </a>
                            <a href="{{ url('company/lowongan/'.$job->id) }}" class="btn btn-sm btn-outline-secondary btn-soft">
                                Kelola Lowongan
                            </a>
                        </div>
                        ...
                        <div class="text-muted small"><i class="bi bi-telephone me-1"></i>{{ $company->phone ?? '-' }}</div>

                        @if (!empty($company->link_website))
                            <div class="mt-2">
                                <a class="btn btn-sm btn-outline-secondary btn-soft" target="_blank" href="{{ $company->link_website }}">
...
<div class="dp-toast" id="dpToast">
    <div class="title" id="dpToastTitle">Berhasil</div>
    <div class="desc" id="dpToastDesc">Status lamaran diperbarui.</div>
</div>
@endsection
